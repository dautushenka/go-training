...
func (s *Bot) ExecuteCommand(command string) {
	...
	case "1":
		fallthrough
	case s.copy("command1"):
		s.sayHello()
		break
	case "2":
		fallthrough
	case s.copy("command2"):
		s.sayTime()
		break
	case "3":
		fallthrough
	case s.copy("command3"):
		s.sayDate()
		break
	case "4":
		fallthrough
	case s.copy("command4"):
		s.sayDay()
		break
	default:
...
func (s *Bot) t(key string, args ...string) string {
	return s.copy(key, args...)
}
